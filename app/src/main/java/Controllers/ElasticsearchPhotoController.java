...
public class ElasticsearchPhotoController {
    ...
    public static class GetPhotosByBodyLocation extends AsyncTask<String,Void,ArrayList<Photo>>{
        ...
        @Override
        protected ArrayList<Photo> doInBackground(String... bodyLocations) {
            setClient();
            String query;
            ...
            while (tryCounter > 0){
                try{
                    ...
                    if (result.isSucceeded()){
                        ...
                        for (Photo photo: photoList){
                            photos.add(photo);
                            Log.d("GetPhotoByBody", "Fetched photo from body location: "+ photo.getUUID());
                        }
                        return photos;
                    } else{
                        Log.d("GetPhotoByBody", "Fetch to get photos from body location failed!");
                    }
                } catch (IOException e1){
